...
public OnPlayerRequestLogin(playerid)
{
	...
	if (rows)
	{
		...
		Dialog_Show(playerid, Dialog_Login, DIALOG_STYLE_PASSWORD, "{FF5555}ViSA > Login", "{FFFFFF}Ol� {FF5555}%s{FFFFFF}, bem-vindo(a) ao ViSA Multiplayer!\n\nEssa conta est� registrada, por favor insira a senha:", "Login", "Sair", ReturnPlayerName(playerid));
	}
	else
	{
		Player[playerid][pID] = -1;
		Dialog_Show(playerid, Dialog_Register, DIALOG_STYLE_INPUT, "{FF5555}ViSA > Registro", "{FFFFFF}Ol� {FF5555}%s{FFFFFF}, bem-vindo(a) ao ViSA Multiplayer!\n\nEssa conta n�o est� registrada, crie uma senha para jogar:", "Registro", "Sair", ReturnPlayerName(playerid));
	}
...
Dialog:Dialog_Register(playerid, response, listitem, inputtext[])
{
	...
	if (!(4 <= strlen(inputtext) <= 20) || strfind(inputtext, " ") != -1)
	{
		Dialog_Show(playerid, Dialog_Register, DIALOG_STYLE_INPUT, "{FF5555}ViSA > Registro", "{FF1414}ERRO: Senha inv�lida!\n\n{FFFFFF}Ol� {FF5555}%s{FFFFFF}, bem-vindo(a) ao ViSA Multiplayer!\n\nEssa conta n�o est� registrada, crie uma senha para jogar:", "Registro", "Sair", ReturnPlayerName(playerid));
		return true;
	}
...
Player_LoadData(playerid)
{
	...
	orm_addvar_string(g_PlayerORM[playerid], Player[playerid][pDiscordCode], 12, "player_discord_ code");
